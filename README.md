# Информация

Покупаю сервера у разных хостингов для тестов и запуска прокси для Telegram

Для прокси используется https://github.com/telemt/telemt  
[Конфиг](https://github.com/Dimasssss/free_telemt_servers/blob/main/config.toml) используемый на всех серверах.

-----

## NKtelecom
- Локация: Netherlands - Amsterdam
- Тариф: AMS-CLOUD-40
- Краткое описание тарифа: 2 vCore, 2 Gb ram, 1 Gbit/s
- Цена в месяц: 4.99$
- Оплачен до: 26 марта

#### Ссылки с SNI
| SNI               | link                                                                                                                |
|-------------------|---------------------------------------------------------------------------------------------------------------------|
| microsoft.com     | tg://proxy?server=195.226.94.42&port=443&secret=eebe3007e927acd147dde12bee8b1a7c936d6963726f736f66742e636f6d        |
| drive.google.com  | tg://proxy?server=195.226.94.42&port=443&secret=eebe3007e927acd147dde12bee8b1a7c9364726976652e676f6f676c652e636f6d  |

-----

## psb.hosting
- Локация: Finland
- Тариф: FI-200
- Краткое описание тарифа: 2 vCore, 4 Gb ram, 1 Gbit/s
- Цена в месяц: 12$
- Оплачен до: 25 марта

#### Ссылки с SNI
| SNI               | link                                                                                                                |
|-------------------|---------------------------------------------------------------------------------------------------------------------|
| microsoft.com     | tg://proxy?server=45.147.28.10&port=443&secret=eebe3007e927acd147dde12bee8b1a7c936d6963726f736f66742e636f6d         |
| drive.google.com  | tg://proxy?server=45.147.28.10&port=443&secret=eebe3007e927acd147dde12bee8b1a7c9364726976652e676f6f676c652e636f6d   |

-----

## koara.io
- Локация: Германия, Франкфурт-на-Майне
- Тариф: DE-2
- Краткое описание тарифа: 2 vCore, 4 Gb ram, 1 Gbit/s
- Цена в месяц: 639 RUB
- Оплачен до: 25 марта

#### Ссылки с SNI
| SNI               | link                                                                                                                |
|-------------------|---------------------------------------------------------------------------------------------------------------------|
| microsoft.com     | tg://proxy?server=108.165.174.245&port=443&secret=eebe3007e927acd147dde12bee8b1a7c936d6963726f736f66742e636f6d      |
| drive.google.com  | tg://proxy?server=108.165.174.245&port=443&secret=eebe3007e927acd147dde12bee8b1a7c9364726976652e676f6f676c652e636f6d|

-----

## landvps.ru
- Локация: Финляндия
- Тариф: FL-2
- Краткое описание тарифа: 2 vCore, 2 Gb ram, 200 Mbps/s
- Цена в месяц: 800 RUB
- Оплачен до: 26 марта

#### Ссылки с SNI
| SNI               | link                                                                                                                |
|-------------------|---------------------------------------------------------------------------------------------------------------------|
| microsoft.com     | tg://proxy?server=185.225.203.236&port=443&secret=eebe3007e927acd147dde12bee8b1a7c936d6963726f736f66742e636f6d      |
| drive.google.com  | tg://proxy?server=185.225.203.236&port=443&secret=eebe3007e927acd147dde12bee8b1a7c9364726976652e676f6f676c652e636f6d|

-----

## 4vps.su
- Локация: Финляндия, Хельсинки
- Тариф: FI-cx21
- Краткое описание тарифа: 2 vCore, 4 Gb ram, 2 Gbit/s
- Цена в месяц: 770 RUB
- Оплачен до: 28 марта

#### Ссылки с SNI
| SNI               | link                                                                                                                |
|-------------------|---------------------------------------------------------------------------------------------------------------------|
| microsoft.com     | tg://proxy?server=213.176.17.198&port=443&secret=eebe3007e927acd147dde12bee8b1a7c936d6963726f736f66742e636f6d       |
| drive.google.com  | tg://proxy?server=213.176.17.198&port=443&secret=eebe3007e927acd147dde12bee8b1a7c9364726976652e676f6f676c652e636f6d |

-----

# Server Metrics 2026-02-27 18:58:57 UTC
- Updated every 5 minutes. 
- Metrics with a value of 0 are not displayed.

## NKtelecom

```
telemt 3.1.2

telemt_uptime_seconds 6643.9 (1h 50m)
telemt_connections_total 520
telemt_connections_bad_total 5
telemt_handshake_timeouts_total 2
telemt_user_connections_total{user="hello"} 474
telemt_user_connections_current{user="hello"} 14
telemt_user_octets_from_client{user="hello"} 5725312 (5.46 MB)
telemt_user_octets_to_client{user="hello"} 90224340 (86.04 MB)
telemt_user_msgs_from_client{user="hello"} 8238
telemt_user_msgs_to_client{user="hello"} 29242
```

## psb.hosting

```
telemt 3.1.2

telemt_uptime_seconds 7135.5 (1h 58m)
telemt_connections_total 156
telemt_connections_bad_total 14
telemt_handshake_timeouts_total 1
telemt_user_connections_total{user="hello"} 136
telemt_user_octets_from_client{user="hello"} 116333 (113.61 KB)
telemt_user_octets_to_client{user="hello"} 161829 (158.04 KB)
telemt_user_msgs_from_client{user="hello"} 416
telemt_user_msgs_to_client{user="hello"} 341
```

## koara.io

```
telemt 3.1.2

telemt_uptime_seconds 6777.6 (1h 52m)
telemt_connections_total 298
telemt_connections_bad_total 7
telemt_user_connections_total{user="hello"} 266
telemt_user_octets_from_client{user="hello"} 1545247 (1.47 MB)
telemt_user_octets_to_client{user="hello"} 8313597 (7.93 MB)
telemt_user_msgs_from_client{user="hello"} 1627
telemt_user_msgs_to_client{user="hello"} 3945
```

## landvps.ru

```
telemt 3.1.2

telemt_uptime_seconds 6890.2 (1h 54m)
telemt_connections_total 324
telemt_connections_bad_total 47
telemt_handshake_timeouts_total 22
telemt_user_connections_total{user="hello"} 265
telemt_user_connections_current{user="hello"} 1
telemt_user_octets_from_client{user="hello"} 928530 (906.77 KB)
telemt_user_octets_to_client{user="hello"} 3033824 (2.89 MB)
telemt_user_msgs_from_client{user="hello"} 2366
telemt_user_msgs_to_client{user="hello"} 2315
```

## 4vps.su

```
telemt 3.1.2

telemt_uptime_seconds 6946.7 (1h 55m)
telemt_connections_total 56
telemt_connections_bad_total 38
telemt_handshake_timeouts_total 1
telemt_user_connections_total{user="hello"} 17
telemt_user_octets_from_client{user="hello"} 73104 (71.39 KB)
telemt_user_octets_to_client{user="hello"} 1954242 (1.86 MB)
telemt_user_msgs_from_client{user="hello"} 260
telemt_user_msgs_to_client{user="hello"} 816
```