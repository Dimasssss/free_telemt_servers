# Информация

Покупаю сервера у разных хостингов для тестов и запуска прокси для Telegram

Для прокси используется https://github.com/telemt/telemt  
[Конфиг](https://github.com/Dimasssss/free_telemt_servers/blob/main/config.toml) используемый на всех серверах.

-----

## NKtelecom
- Локация: Netherlands - Amsterdam
- Тариф: AMS-CLOUD-40
- Краткое описание тарифа: 2 vCore, 2 Gb ram, 1 Gbit/s
- Цена в месяц: 4.99$
- Оплачен до: 26 марта
- Ссылка: `tg://proxy?server=195.226.94.42&port=443&secret=eebe3007e927acd147dde12bee8b1a7c9364726976652e676f6f676c652e636f6d`

-----

## psb.hosting
- Локация: Finland
- Тариф: FI-200
- Краткое описание тарифа: 2 vCore, 4 Gb ram, 1 Gbit/s
- Цена в месяц: 12$
- Оплачен до: 25 марта
- Ссылка: `tg://proxy?server=45.147.28.10&port=443&secret=eebe3007e927acd147dde12bee8b1a7c9364726976652e676f6f676c652e636f6d`

-----

## koara.io
- Локация: Германия, Франкфурт-на-Майне
- Тариф: DE-2
- Краткое описание тарифа: 2 vCore, 4 Gb ram, 1 Gbit/s
- Цена в месяц: 639 RUB
- Оплачен до: 25 марта
- Ссылка: `tg://proxy?server=108.165.174.245&port=443&secret=eebe3007e927acd147dde12bee8b1a7c9364726976652e676f6f676c652e636f6d`

-----

## landvps.ru
- Локация: Финляндия
- Тариф: FL-2
- Краткое описание тарифа: 2 vCore, 2 Gb ram, 200 Mbps/s
- Цена в месяц: 800 RUB
- Оплачен до: 26 марта
- Ссылка: `tg://proxy?server=185.225.203.236&port=443&secret=eebe3007e927acd147dde12bee8b1a7c9364726976652e676f6f676c652e636f6d`

-----

# Server Metrics 2026-03-01 12:58:46 UTC
- Updated every 5 minutes. 
- Metrics with a value of 0 are not displayed.

## NKtelecom

```
telemt 3.1.3

telemt_uptime_seconds 6731.6 (1h 52m)
telemt_telemetry_core_enabled 1
telemt_telemetry_user_enabled 1
telemt_telemetry_me_level{level="normal"} 1
telemt_connections_total 4751
telemt_connections_bad_total 20
telemt_handshake_timeouts_total 3
telemt_user_connections_total{user="hello"} 4651
telemt_user_connections_current{user="hello"} 13
telemt_user_octets_from_client{user="hello"} 43843543 (41.81 MB)
telemt_user_octets_to_client{user="hello"} 645170806 (615.28 MB)
telemt_user_msgs_from_client{user="hello"} 40958
telemt_user_msgs_to_client{user="hello"} 121449
```

## psb.hosting

```
telemt 3.1.3

telemt_uptime_seconds 6740.0 (1h 52m)
telemt_telemetry_core_enabled 1
telemt_telemetry_user_enabled 1
telemt_telemetry_me_level{level="normal"} 1
telemt_connections_total 344
telemt_connections_bad_total 56
telemt_handshake_timeouts_total 2
telemt_user_connections_total{user="hello"} 280
telemt_user_octets_from_client{user="hello"} 6662008 (6.35 MB)
telemt_user_octets_to_client{user="hello"} 42938822 (40.95 MB)
telemt_user_msgs_from_client{user="hello"} 4926
telemt_user_msgs_to_client{user="hello"} 10736
```

## koara.io

```
telemt 3.1.3

telemt_uptime_seconds 6774.6 (1h 52m)
telemt_telemetry_core_enabled 1
telemt_telemetry_user_enabled 1
telemt_telemetry_me_level{level="normal"} 1
telemt_connections_total 98
telemt_connections_bad_total 29
telemt_user_connections_total{user="hello"} 60
telemt_user_octets_from_client{user="hello"} 109552 (106.98 KB)
telemt_user_octets_to_client{user="hello"} 2752126 (2.62 MB)
telemt_user_msgs_from_client{user="hello"} 238
telemt_user_msgs_to_client{user="hello"} 792
```

## landvps.ru

```
telemt 3.1.3

telemt_uptime_seconds 6551.2 (1h 49m)
telemt_telemetry_core_enabled 1
telemt_telemetry_user_enabled 1
telemt_telemetry_me_level{level="normal"} 1
telemt_connections_total 571
telemt_connections_bad_total 11
telemt_handshake_timeouts_total 1
telemt_user_connections_total{user="hello"} 59
telemt_user_octets_from_client{user="hello"} 840250 (820.56 KB)
telemt_user_octets_to_client{user="hello"} 11878274 (11.33 MB)
telemt_user_msgs_from_client{user="hello"} 1900
telemt_user_msgs_to_client{user="hello"} 4946
```